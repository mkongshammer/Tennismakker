...
This is a generic migration path, not a verified Resasports API integration. Actual Resasports export headers/availability need a sample from the club/vendor. Existing iCalendar integration remains the option for supported ongoing availability feeds. For cutover, freeze changes in the old system, import and compare, then select RacketBuddy as the booking system; do not run two independent systems accepting the same slots.
