...
export function AdminNavigation({ section, facility }: { section: AdminSection; facility: string }) {
  ...
  const menu = <nav aria-label="Klubadministration" className="space-y-5">
    {['Daglig drift', 'Opsætning'].map(group => <div key={group}>
      ...
      <ul className="grid grid-cols-1 gap-1 sm:grid-cols-2 lg:grid-cols-1">{ADMIN_PAGES.filter(p => p.group === group).map(p => <li key={p.id}>
