...
import { SUPERADMIN_PAGES, superadminHref, type SuperadminSection } from "../../lib/superadmin-navigation";

export default async function SuperadminPageContent({ section }: { section: SuperadminSection }) {
  ...
  const pct = section === "klubber" ? Math.round((await getSettings()).commissionPct * 100) : 0;
  const [pending, decided, orders, domainClubs, leads] = await Promise.all([
    section === "klubber" ? db.club.findMany({
    ...
    section === "hjemmesider" ? db.websiteOrder.findMany({
      where: { status: { notIn: ["LIVE", "CANCELLED"] } },
      orderBy: { createdAt: "asc" },
    }) : Promise.resolve([]),
    section === "domaener" ? db.club.findMany({
      where: { status: "APPROVED" },
      select: { id: true, name: true, customDomain: true, domainStatus: true },
      orderBy: { name: "asc" },
    }) : Promise.resolve([]),
    section === "henvendelser" ? db.clubLead.findMany({
  ...
  return (
    <div className="space-y-10">
      <div>
        <h1 className="display text-3xl">{SUPERADMIN_PAGES.find(p => p.id === section)!.label}</h1>
        <p className="mt-2 text-slate">{SUPERADMIN_PAGES.find(p => p.id === section)!.description}</p>
      </div>
      {section === 'oversigt' && <>
        <div className="grid gap-3 sm:grid-cols-2">
          {SUPERADMIN_PAGES.filter(p => ['henvendelser', 'klubber', 'opret-klub', 'oekonomi'].includes(p.id)).map(p =>
            <Link key={p.id} href={superadminHref(p.id)} prefetch={false} className="card transition-colors hover:bg-mist focus-visible:outline focus-visible:outline-2 focus-visible:outline-court">
              <span className="font-bold">{p.label} →</span><p className="mt-1 text-sm text-slate">{p.description}</p>
            </Link>)}
        </div>
        <Overblik />
      </>}
      {section === 'traenere' && <CoachPhotos />}
      {section === 'aktivitet' && <Aktivitet />}
      {section === 'oekonomi' && <Oekonomi />}
      {section === 'adgang' && <SuperadminAccess />}
            ...
            {leads.map((lead: any) => (
              <li key={lead.id} className="card">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Link
                    href={`/superadmin/opret-klub?leadId=${lead.id}&clubName=${encodeURIComponent(lead.clubName)}&city=${encodeURIComponent(lead.city)}&adminName=${encodeURIComponent(lead.contactName)}&adminEmail=${encodeURIComponent(lead.email)}`}
      ...
      <section id="opret-klub">
        ...
        <p className="mb-4 text-sm text-slate">
          ...
          Kommer du fra en klubhenvendelse, er felterne udfyldt på forhånd.
        </p>
        <CreateClubForm />
      </section>)}
